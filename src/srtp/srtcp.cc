...
rtp_error_t uvgrtp::srtcp::encrypt(uint32_t ssrc, uint64_t seq, uint8_t *buffer, size_t len)
{
    ...
    if (create_iv(iv, ssrc, seq, srtp_ctx_->local_key_ctx.session.salt_key) != RTP_OK) {
...
{
    auto hmac_sha1 = uvgrtp::crypto::hmac::sha1(srtp_ctx_->local_key_ctx.session.auth_key, UVG_AES_KEY_LENGTH);
...
{
    uint8_t digest[10] = { 0 };
    auto hmac_sha1     = uvgrtp::crypto::hmac::sha1(srtp_ctx_->remote_key_ctx.session.auth_key, UVG_AES_KEY_LENGTH);
...
rtp_error_t uvgrtp::srtcp::decrypt(uint32_t ssrc, uint32_t seq, uint8_t *buffer, size_t size)
{
    ...
    if (create_iv(iv, ssrc, seq, srtp_ctx_->remote_key_ctx.session.salt_key) != RTP_OK) {
        UVG_LOG_ERROR("Failed to create IV, unable to encrypt the RTP packet!");
        return RTP_INVALID_VALUE;
    }

    uvgrtp::crypto::aes::ctr ctr(srtp_ctx_->remote_key_ctx.session.enc_key, srtp_ctx_->n_e, iv);
