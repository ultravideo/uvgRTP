...
int main(void)
{
    ...
    /* To enable interoperability between RTP libraries, kvzRTP won't fragment generic frames by default.
     *
     ...
     * received kvzRTP constructs one full RTP frame from the fragments and returns the frame to user.
     *
     * See sending.cc for more details about create_stream() */
    kvz_rtp::media_stream *send = sess->create_stream(8888, 8889, RTP_FORMAT_GENERIC, RCE_FRAGMENT_GENERIC);
    kvz_rtp::media_stream *recv = sess->create_stream(8889, 8888, RTP_FORMAT_GENERIC, RCE_FRAGMENT_GENERIC);

    /* Notice that PAYLOAD_MAXLEN > MTU (4096 > 1500).
     *
     * kvzRTP fragments all generic input frames that are larger than 1500 and in the receiving end,
